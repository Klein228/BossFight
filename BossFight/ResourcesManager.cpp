...
static const std::vector<AtlasResInfo> atlas_info_list = {
	...
	{"skill",_T(R"(resources\enemy\skill\%d.png)"), 9},
...
void ResourcesManager::flip_atlas(const std::string& src_id, const std::string& dst_id)
{
	...
	{
		IMAGE img_flipped;
		flip_image(src_atlas->get_img_at(i), &img_flipped);
...
void ResourcesManager::flip_image(IMAGE* src_image, IMAGE* dst_image, int num_h)
{
	...
	DWORD* src_buf = (DWORD*)GetImageBuffer(src_image);
	DWORD* dst_buf = (DWORD*)GetImageBuffer(dst_image);
	for (int i = 0; i < num_h; i++)
	{
		...
		for (int y = 0; y < h; y++)
		{
			...
			{
				int src_idx = y * w + x;
				int dst_idx =  y * w + x_right - (x - x_left);
...
void ResourcesManager::load()
{
	...
	load_audio(_T(R"(resources\audio\enemy_throw_skills.mp3)"), _T("enemy_throw_skills"));
